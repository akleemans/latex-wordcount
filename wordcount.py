...
ignoredList = ["document", "tiny", "scriptsize", "footnotesize", "small", "normalsize", "large", "Large", "LARGE",
               "huge", "huge", "flushleft", "center", "flushright"]

global wordNos

wordNos = [i for i in range(len(words))]
...
for i in range(len(words)):
    ...
    # named functions
    elif words[i].find("\\") == 0 and words[i].count("{") == words[i].count("}") and words[i].count("[") == words[
        i].count("]"):
        try:
            wordNos.remove(i)
        except ValueError:
...
print("{} words".format(len(wordNos)))
